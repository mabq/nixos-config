# Manual Installation (BIOS)

Create a bootable USB — see [here](https://nixos.org/manual/nixos/stable/#ch-installation).

Login as root (`sudo -i`), change its password (`passwd`) and ssh into the machine (`ssh root@<IP>`) from anyther machine where you have access to this document, that way you can copy/paste most commands from this document.
